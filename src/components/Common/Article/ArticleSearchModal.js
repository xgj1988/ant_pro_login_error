...
import { BackTop, Button, Col, Form, message, Row, Switch, Tooltip } from 'antd';
...
@connect(({loading}) => ({
  loading: loading.effects['article/list'],
}))
@Form.create()
export default class ArticleSearchModal extends PureComponent {
  constructor(props) {
    super(props);
  ...
  render() {
    const columns = [{
      ...
      render: (value, record) => {
        ...
        return (
          <span style={{color}}>
            {utils.getMsg(`article_state_${value}`)}
          </span>
        );
      },
    }, {
      ...
      render: (value, record) => {
        if (!record.autoCreate) {
          ...
          return (
            <span style={{color}}>
              {value ? (
                <a style={{color}} href={record.adUrl} rel="noopener noreferrer" target="_blank">
                  是
                </a>
              ) : '否'}
            </span>
