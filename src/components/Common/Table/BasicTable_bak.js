...
export default class BasicTable extends PureComponent {
  ...
  componentDidMount() {
    const { totalRow = false } = this.props;
    if (totalRow) {
      // eslint-disable-next-line
      const dom = ReactDOM.findDOMNode(this);
      const table = dom.querySelectorAll('.ant-table-body');
      const mainTable = table[0];
      mainTable.style.overflowX = 'hidden';
      const footerTable = table[1];
      footerTable.addEventListener('scroll', () => {
        mainTable.scrollLeft = footerTable.scrollLeft;
      });
    }
  }
  ...
  calcScrollFn = columns => {
    const { calcScrollX = true, calcScrollY = true, scroll = {}, size = 'small' } = this.props;
    let result = scroll;
    if (calcScrollX) {
      if (scroll) {
        result = { ...scroll };
  ...
  alignment = () => {
    let ths = '';
    const tdsWdith = [];
    const { autoAlign = true } = this.props;
    if (autoAlign && this.table) {
      const [thread] = this.table.getElementsByClassName('ant-table-thead');
      if (thread) {
        ths = thread.childNodes[0].childNodes;
      }
      const table = this.table.getElementsByClassName('ant-table-tbody')[0];
      if (table) {
        const tr = table.getElementsByClassName('ant-table-row')[0];
        if (tr) {
          const tds = tr.childNodes;
          for (let i = 0; i < tds.length; i += 1) {
            tdsWdith.push(utils.getStyle(tds[i], 'width'));
          }
          for (let i = 0; i < tds.length; i += 1) {
            ths[i].style.minWidth = tdsWdith[i];
          }
        }
      }


      const fixedTable = this.table.getElementsByClassName('ant-table-fixed-left');
      for (let j = 0; j < fixedTable.length; j += 1) {
        let fixedThread;
        let fixedThs;
        if (fixedTable.length > 0) {
          // fixedThread = fixedTable[j].getElementsByClassName('ant-table-thead')[0]
          // if (!fixedThread) {
          //   fixedThread = fixedTable[j].getElementsByClassName('ant-table-row')[0]
          // }
          [fixedThread] = fixedTable[j].getElementsByTagName('tr');
          if (fixedThread) {
            fixedThs = fixedThread.childNodes;
            for (let i = 0; i < fixedThs.length; i += 1) {
              fixedThs[i].style.minWidth = tdsWdith[i];
              fixedThs[i].style.width = tdsWdith[i];
            }
          }
        }
      }
      ...
      // 合计宽度赋值上一个表头的宽度
      const dom = this.table.getElementsByClassName('ant-table-footer');
      if (dom && dom[0]) {
        const tr = dom[0].getElementsByClassName('ant-table-row')[0];
        const tds = tr.childNodes;
        for (let i = 0; i < tds.length; i += 1) {
          tds[i].style.minWidth = tdsWdith[i];
          tds[i].style.width = tdsWdith[i];
        }
      }
    }
  };
  ...
  addLastColumn = () => {
    // 最后一列
    const { lastColumn = true } = this.props;
    const { columns } = this.state;
    const antTableContentElmt = document.getElementsByClassName('ant-table-content')[0];
    let tableWidth = null;
    if (antTableContentElmt) {
      tableWidth = antTableContentElmt.clientWidth;
    }
    const newColumns = [...columns];
    const columnsLength = newColumns.length;
    let columnsWidth = 0;
    let lastColumnWidth = 0;
    for (let i = 0; i < columnsLength; i += 1) {
      columnsWidth += parseInt(newColumns[i].width, 0);
    }
    if (newColumns[columnsLength - 1].lastColumn) {
      columnsWidth -= newColumns[columnsLength - 1].width;
      newColumns.splice(-1, 1);
    }
    if (tableWidth && (tableWidth > columnsWidth)) {
      lastColumnWidth = tableWidth - columnsWidth + parseInt(newColumns[0].width, 0);
    } else {
      lastColumnWidth = 100;
    }
    if (lastColumn) {
    ...
    return (
      <Fragment >
        <Table
          ...
          ref={(c) => {
            // eslint-disable-next-line
            this.table = ReactDOM.findDOMNode(c);
          }}
        />
        {hasEditButton ? (
          <EditColumnModal
