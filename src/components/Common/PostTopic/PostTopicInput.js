...
import style from './PostTopicInput.less';
import Ellipsis from '../../Ellipsis';
import PostTopicSearchModal from './PostTopicSearchModal';
 ...
 */
@connect()
export default class PostTopicInput extends PureComponent {
  constructor(props) {
    super(props);
    this.readOnly = false;
    this.delimiter = ',';
  ...
  generateKeyword = (addDelimiter = false) => {
    ...
    this.value.forEach(item => {
      result = `${result}${this.delimiter}${item.title}`;
    });
    ...
    const options = dataSource.map(option => {
      return (
        <Option key={option.id} value={utils.uuid()} item={option}>
          {option.highlightTitle ? (
            <Ellipsis lines={1} tooltip>
              <span
                dangerouslySetInnerHTML={{ __html: option.highlightTitle }}
              />
            </Ellipsis>
          ) : (
            <Ellipsis lines={1} tooltip>
