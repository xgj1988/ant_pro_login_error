...
export default class TabPostTopicContent extends PureComponent {
  ...
  handleRebuildProductMapping = e => {
    e.preventDefault();
    const { handleRebuildIndexSubmit, currentPostMapping } = this.props;
    handleRebuildIndexSubmit(currentPostMapping);
  };
    ...
    return (
      <div>
        <Form onSubmit={this.handlePostSubmit}>
          <FormItem {...Constant.formItemLayout} label="帖子Mapping">
            {getFieldDecorator('memberValue', {
              initialValue: currentPostMapping.value,
              rules: [
                {
                  required: true,
          ...
          <PermissionAuth permissions="system:search_engine_mapping:rebuild">
            <FormItem {...Constant.formItemLayout} label="重建帖子索引">
              <Button
